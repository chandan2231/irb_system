...
const AssignProtocolToMember = ({
  open,
  onClose,
  addAssignedMemberData,
  title,
  protocolDetails,
}) => {
  ...
  const { activeVotingMemberList, loading, error, memberAssignedList } =
    useSelector((state) => ({
  ...
  const getContent = () => (
    <>
      ...
      {memberAssignedList !== null &&
        memberAssignedList.data.length > 0 &&
        memberAssignedList.data.map((list, index) => (
          <span key={index}>
            {list.name}
            {index < memberAssignedList.data.length - 1 && ", "}
          </span>
        ))}
      <Box sx={modalStyles.inputFields}>
        ...
        {memberSelectionError && (
          <span style={{ color: "red" }}>{memberSelectionError}</span>
        )}
      </Box>
    </>
  );
