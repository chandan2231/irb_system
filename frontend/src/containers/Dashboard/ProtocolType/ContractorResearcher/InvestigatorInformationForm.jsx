...
const investigatorInfoSchema = yup.object().shape({
    ...
    investigator_research_number: yup.string().required("Number of supervised studies is required"),
    pending_or_active_research: yup.string().required("Please specify if there are any pending or active research restrictions"),
    pending_or_active_research_explain: yup.string().when('pending_or_active_research', {
        is: () => 'Yes',
        then: () => yup.string().required("Explanation is required if there are pending or active research restrictions"),
    }),
    site_fwp: yup.string().required("Please specify if your site has an FWA"),
    fwa_number: yup.string().when('site_fwp', {
        is: () => 'Yes',
        then: () => yup.string().required("FWA number is required if the site has an FWA"),
    }),
    cv_files: yup.mixed().required("Please upload investigator and sub-investigator CV"),
...
function InvestigatorInformationForm({ protocolTypeDetails, investigatorInformation }) {
    ...
    useEffect(() => {
        if (investigatorInformation) {
            ...
            setShowOtherQuestion(investigatorInformation.training_completed.includes('8') ? true : false)
            setOtherQuestionSelection(investigatorInformation.training_completed.includes('8') ? '8' : '')
        }
            ...
            if (isValid === true) {
                ...
                if (!formData.cv_files) {
                    ...
                    for (let file of formData.cv_files) {
                        let id = await uploadFile(file, { protocolId: formData.protocol_id, createdBy: formData.created_by, protocolType: protocolTypeDetails.researchType, informationType: 'investigator_information', documentName: 'investigator_cv' })
                        cv_files.push(id)
                    }
                    if (formData.medical_license) {
                        for (let file of formData.medical_license) {
                            let id = await uploadFile(file, { protocolId: formData.protocol_id, createdBy: formData.created_by, protocolType: protocolTypeDetails.researchType, informationType: 'investigator_information', documentName: 'medical_license' })
                            medical_license.push(id)
                        }
                    }
                    if (formData.training_certificates) {
                        for (let file of formData.training_certificates) {
                            let id = await uploadFile(file, { protocolId: formData.protocol_id, createdBy: formData.created_by, protocolType: protocolTypeDetails.researchType, informationType: 'investigator_information', documentName: 'training_certificates' })
