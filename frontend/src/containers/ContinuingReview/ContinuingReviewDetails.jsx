...
const ContinuingReviewDetails = () => {
    ...
    React.useEffect(() => {
        let data = { protocolId: continuinReviewDetails?.protocolId, protocolType: continuinReviewDetails?.researchType }
    ...
    const { continuinReviewDetailsById, loading, error } = useSelector(
        state => ({
            ...
            continuinReviewDetailsById: state.continuinReview.continuinReviewDetailsById,
        ...
        <Box sx={{ width: '100%' }}>
            ...
            <CustomTabPanel value={value} index={1}>
                <InformedConsentProcess continuinReviewDetails={continuinReviewDetails}
                    informedConsentProcess={continuinReviewDetailsById?.informed_consent_process} />
            </CustomTabPanel>
            <CustomTabPanel value={value} index={2}>
                <InvestigatorInstitutionInfo continuinReviewDetails={continuinReviewDetails}
                    investigatorInstitutionInfo={continuinReviewDetailsById?.investigator_instuation_info} />
            </CustomTabPanel>
