...
function AdminProtocolAmendmentRequestDetails() {
    ...
    const amendDocumentTypeArr = protocolAmendmentRequestById && protocolAmendmentRequestById.protocol_amendment_request?.amend_document?.split(",");
    return (
        <Box sx={{ width: '100%' }}>
                                        ...
                                        <FormGroup name="amend_document">
                                            {
                                                amendDocumentType.map((appliedChanges, index) => {
                                                    return(<FormControlLabel key={index} control={<Checkbox />} label={appliedChanges.label} value={appliedChanges.value} checked={amendDocumentTypeArr?.find(id => Number(id) === Number(appliedChanges.value))} />)
                                                })
                                            }
                                        </FormGroup>
                                    </FormControl>
                                </Form.Group>
                                {
                                    amendDocumentTypeArr?.includes('4') && (
                                        <Form.Group as={Col} controlId="validationFormik03" className='mt-mb-20'>
                                            ...
                                            <p className='explain_text'>{protocolAmendmentRequestById.protocol_amendment_request?.amend_document_explain}</p>
                                        </Form.Group>
                                    )
                                }
                                <Box sx={{ flexGrow: 1 }}>
                                    <Form.Group as={Col} controlId="validationFormik010" className='mt-mb-20'>
                                        <InputLabel id="demo-simple-select-autowidth-label">Uploaded redlined document(s) here</InputLabel>
                                        {/* <div className='highlight-text'>Please note: Documents must be in WORD version showing “track changes” </div> */}
                                        {
                                            protocolAmendmentRequestById.protocol_amendment_request?.documents?.length > 0 && protocolAmendmentRequestById.protocol_amendment_request?.documents?.map((docList, index) => {
                                    ...
                                    <Form.Group as={Col} controlId="validationFormik03" className='mt-mb-20'>
                                        <Box sx={{ width: '100%', maxWidth: '100%' }}>
                                            ...
                                            <p className='explain_text'>{protocolAmendmentRequestById.protocol_amendment_request?.describe_change_request}</p>
                                        </Box>
                                    </Form.Group>
                                    ...
                                    <Form.Group as={Col} controlId="validationFormik03" className='mt-mb-20'>
                                        <Box sx={{ width: '100%', maxWidth: '100%' }}>
                                            ...
                                            <p className='explain_text'>{protocolAmendmentRequestById.protocol_amendment_request?.describe_reasoning}</p>
                                        </Box>
                                    </Form.Group>
                                </Box>
