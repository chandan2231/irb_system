...
import { fetchUsersList, changeUserStatus, resetUserPassword } from "../../../services/Admin/UsersListService";
...
function UsersList() {
    ...
    const resetPasswordData = (data) => {
        ...
        dispatch(resetUserPassword(dataObject))
        .then(data => {
            if (data.payload.status === 200) {
    ...
    return (
        <Box m={theme.layoutContainer.layoutSection}>
            ...
            <Box>
                <ChangePassword open={passwordChangeOpen} onClose={() => setPasswordChangeOpen(false)} addNewData={resetPasswordData} title='Reset User Password'/>
            </Box>
            <Box sx={{ mt: 5 }}>
                <DataGrid
                    rows={allUsersList}
