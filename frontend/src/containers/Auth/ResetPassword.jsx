...
function ResetPassword() {
    ...
    useEffect(() => {
        ...
        dispatch(validateUser(dataObject))
        .then(data => {
                                ...
                                <CardContent>
                                    {
                                        passwordResetSuccess === true ? (
                                            <>
                                                <Typography sx={{mt: 2, mb: 5}}><h4 sx={styles.title} className="success_msg">You have reset your password successfully</h4></Typography>
                                                <br />
                                                <Typography sx={{mt: 2, mb: 5}}><h4 sx={styles.title}>You will be redirected in 10 seconds {timer}</h4></Typography>
                                            </>
                                        ) : passwordResetSuccess === false ? (
                                            <Typography sx={{mt: 2, mb: 5}}><h4 sx={styles.title} className="error_msg">Something went wrong</h4></Typography>
                                        ) : (
                                            <></>
                                        )
                                    }
                                    {
                                        (passwordResetSuccess === false || passwordResetSuccess === '') ? (
                                            <>
                                                <Typography sx={{mt: 2, mb: 5}}><h2 sx={styles.title}>Reset Password</h2></Typography>
                                                ...
                                                <FormControl fullWidth variant="outlined">
                                                    ...
                                                    <OutlinedInput
                                                        ...
                                                        id="outlined-adornment-password"
                                                        type={showPassword ? 'text' : 'password'}
                                                        onChange={(event) => handleChange({ ...values, confirm_password: event.target.value })}
                                                        endAdornment={
                                                            <InputAdornment position="end">
                                                                <IconButton
                                                                    aria-label="toggle password visibility"
                                                    ...
                                                    />
                                                    {Object.keys(errors).length > 0 && <div className="error_msg">{errors?.confirm_password?.message}</div>}
                                                </FormControl>
                                                <Button variant="contained" type='submit' size='large' sx={styles.buttonStyle}>Submit</Button>
                                            </>
                                        ) : (
                                            <></> 
                                        )
