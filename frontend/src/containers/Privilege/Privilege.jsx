...
import { fetchPrivilegeList, createPrivilege, changeStatus } from "../../services/Privilege/PrivilegeService";
...
function Privilege(){
    ...
    var totalElements = 0;
    const { privilegeList, loading, error, createdPrivilege } = useSelector(
        state => ({
            error: state.privilege.error,
            privilegeList: state.privilege.privilegeList,
            loading: state.privilege.loading,
            createdPrivilege: state.privilege.createdPrivilege
        })
    );
    useEffect(() => {
        const data = { page: paginationModel.page, size: paginationModel.pageSize };
        dispatch(fetchPrivilegeList(data));
    }, [dispatch, paginationModel.page, paginationModel.pageSize]);

    if(privilegeList !== '' && privilegeList?.content?.length > 0){
        totalElements = privilegeList.totalElements;
    }
    ...
    useEffect(() => {
        const pListArr = []
        if(privilegeList && privilegeList?.content?.length > 0) {
            privilegeList.content.map((rList, index) => {
            let objectData = {
                    id: rList.id,
                    name:  rList.name,
                    description:  rList.description,
                    market:  rList.market?.name,
                    code:  rList.market?.code,
    ...
    const addNewData = (data) => {
        const {name, market_id, application_id, description} = data
        let dataObject = {
        ...
        dispatch(createPrivilege(dataObject));
        setOpen(false);
    };
    ...
    useEffect(() => {
        ...
        if(createdPrivilege){
            ...
            dispatch(fetchPrivilegeList(paginationData));
        }
    }, [createdPrivilege, paginationModel.page, paginationModel.pageSize])
    ...
    return(
        <Box m={theme.layoutContainer.layoutSection}>
            <Box>
                <Grid container spacing={2}>
                    <Grid item xs={5} sm={5} md={8} lg={8}>
                        <Typography variant="h5" mb={2}>Privilege</Typography>
                    </Grid>
            ...
            <Box>
                <AddPrivilege open={open} onClose={() => setOpen(false)} addNewData={addNewData}/>
            </Box>
            {/* <Box sx={{mt: 5}}>
                        ...
                        <Select
                            ...
                            renderValue={(selected) => {
                                if (selected.length === 0) {
                                    return <em>Search by: Privilege</em>;
                                }
