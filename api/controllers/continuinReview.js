...
export const saveInformedConsent = (req, res) => {
  ...
  db.query(checkQuery, [req.body.protocol_id], (err, data) => {
    ...
    if (data.length > 0) {
      const updateQuery =
        'UPDATE informed_consent_process SET ' +
        '`challenges_faced` = ?, `challenges_faced_explain` = ?, `changes_consent` = ?, `changes_consent_explain` = ?, ' +
        '`ensuring_list` = ?, `ensuring_list_explain` = ?, `icf_version` = ?, `performing_consent` = ?, ' +
      ...
      db.query(updateQuery, values, (err, data) => {
        if (err) {
          return res.status(500).json({
            message: 'Error updating informed consent process',
            error: err
          })
        } else {
          return res.json({
            status: 200,
            msg: 'Informed Consent Process has been updated successfully'
          })
        }
      })
    } else {
