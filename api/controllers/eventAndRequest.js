...
export const saveProtocolAmendmentRequest = (req, res) => {
  var datetime = new Date()
  const que =
    'insert into protocol_amendment_request (`protocol_id`, `protocol_type`, `protocol_number`, `amend_document`, `amend_document_explain`, `describe_change_request`, `describe_reasoning`, `person_name`, `email`, `phone`,  `your_name`, `updated_at`, `created_at`, `created_by`) value (?)'
  const values = [
    req.body.protocol_id,
    req.body.protocol_type,
    req.body.protocol_number,
  ...
  db.query(que, [values], (err, data) => {
    if (err) {
      ...
      result.status = 200
      result.msg =
        'Protocol Amendment Request Details has been saved successfully'
      return res.json(result)
    }
  })
}
